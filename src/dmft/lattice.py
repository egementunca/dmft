...
import numpy as np
from .matsubara import fermi_function, pole_matsubara_sum
...
def lattice_h_sector_gf(iw: np.ndarray, G_dd_lat: np.ndarray,
    ...
    G_hd = np.conj(W)[:, None] * denom * G_dd_lat[None, :]
...
def lattice_correlators(iw: np.ndarray, G_dd_lat: np.ndarray,
    ...
    for l in range(M_h):
        ...
        # Second term: |W_l|^2 * G_dd / (iw - eta_l)^2
        # This must be summed numerically (G_dd has no simple pole form on Bethe)
        # (1/beta) sum_{all n} F(iw_n) with F = |W|^2 * G_dd / (iw - eta)^2
        # Use: sum_{all n} F(iw_n) = 2 * Re[sum_{n>=0} F(iw_n)]
        F_l = np.abs(W[l])**2 * G_dd_lat / (iw - eta[l])**2
        term2 = (1.0 / beta) * 2.0 * np.sum(F_l).real

        hh_corr[l] = term1 + term2

        # <d^dag h_l> = (1/beta) sum_n G^{hd}_l(iw_n)
        # G^{hd}_l = W_l^* / (iw - eta_l) * G_dd
        F_dh = np.conj(W[l]) * G_dd_lat / (iw - eta[l])
        dh_corr[l] = (1.0 / beta) * 2.0 * np.sum(F_dh).real

    return {'hh': hh_corr, 'dh': dh_corr}
