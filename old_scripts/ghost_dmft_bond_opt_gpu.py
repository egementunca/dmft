...
GPU-accelerated version: build_H2 sector diagonalizations run on CUDA
via CuPy. Falls back to CPU automatically if CuPy is unavailable.
Pass --no-gpu to force CPU execution.
...
Usage:
  python ghost_dmft_bond_opt_gpu.py --M 1 --U 1.3 --mode both
  python ghost_dmft_bond_opt_gpu.py --M 2 --U 1.3 --mode both
  python ghost_dmft_bond_opt_gpu.py --M 2 --U 1.3 --mode both --no-gpu
"""
...
_GPU_DIM_THRESHOLD = 256


def _init_gpu(requested):
    """Resolve GPU availability and set _gpu_state[0]. Returns True if GPU active."""
    if not requested:
        return False
...
def _eigh(H):
    ...
    Routes to cuSOLVER (CuPy) for large sectors when GPU is active,
    otherwise uses numpy LAPACK.
    """
...
def build_H2(beta, eps, V, epsb, Bg, dmu, M, U, mu, t, ed=0.0):
    ...
    Sector diagonalizations are dispatched to GPU (_eigh) when _USE_GPU is True
    and the sector dimension exceeds _GPU_DIM_THRESHOLD.
    """
    nps   = 2 + 3*M
    ed_eff = ed - mu - dmu
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Ghost-DMFT M=1 or 2, square lattice (GPU)')
    ...
    parser.add_argument('--no-gpu',  action='store_true',           help='disable GPU, force CPU')
    ...
    if gpu_active:
        ...
        print(f'GPU: {cp.cuda.runtime.getDeviceProperties(dev.id)["name"].decode()} '
              f'(dim threshold: {_GPU_DIM_THRESHOLD})')
    else:
    ...
    if args.out is None:
        args.out = f'ghost_dmft_square_M{args.M}_U{args.U}_t{args.t}_{args.mode}_gpu.dat'
