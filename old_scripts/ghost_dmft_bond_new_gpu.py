#!/usr/bin/env python3
"""
Ghost-DMFT bond (BPK) scheme — independent M per g-ghost family (GPU version).
...
GPU-accelerated with CuPy for eigendecomposition.
"""
...
# Select numpy/cupy and eigh function
if USE_GPU:
    xp = cp
    def eigh_gpu(H):
        H_gpu = cp.asarray(H)
        ev, U = cp_linalg.eigh(H_gpu)
        return cp.asnumpy(ev), cp.asnumpy(U)
    eigh = eigh_gpu
else:
    xp = np
...
if __name__ == '__main__':
    ...
    print(f'\nGhost-DMFT bond (GPU)  M1g={M1g} M2g={M2g} Mbg={Mbg}  '
